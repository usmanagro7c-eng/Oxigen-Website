...
/**
 * Stateless signed session cookie for the admin dashboard backend.
 *
 ...
 * `sid` cookie alone. This module signs our own `oxi_admin_session` cookie at
 * login time (HMAC + expiry) so session hydration keeps working without an
 * ERPNext round-trip.
 *
 * The cookie name is intentionally distinct from the website backend's
 * `oxi_session` — cookies are scoped per host (not per port), so a shared
 * name would let one app's session clobber the other's.
 */

const SESSION_SECRET =
  process.env["SESSION_SECRET"] ??
  process.env["WEBHOOK_SECRET"] ??
  "oxigen-admin-dev-session-secret";
...
/** Read + verify the `oxi_admin_session` cookie. Returns the email or null. */
export function parseSessionEmail(cookieHeader?: string): string | null {
  if (!cookieHeader) return null;
  const match = /(?:^|;\s*)oxi_admin_session=([^;]+)/.exec(cookieHeader);
