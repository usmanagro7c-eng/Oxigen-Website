...
function SettingsPage() {
  ...
  const [notif, setNotif] = useState({ email: true, push: true, marketing: false });
  const [twofa, setTwofa] = useState(true);

  const save = (e: React.FormEvent) => {
    e.preventDefault();
    setStatus("loading");
    setTimeout(() => { setStatus("saved"); setTimeout(() => setStatus("idle"), 1500); }, 800);
  };
          ...
          <section id="organization">
            <GlassCard title="Organization" desc="Public details about your OxiGen healthcare brand.">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <FieldGroup label="Brand name"><Input defaultValue="OxiGen Healthcare" /></FieldGroup>
                <FieldGroup label="Website"><Input type="url" defaultValue="https://oxigen.pk" /></FieldGroup>
                <FieldGroup label="Support email"><Input type="email" defaultValue="support@oxigen.pk" /></FieldGroup>
            ...
            <GlassCard title="Preferences" desc="Localization and currency defaults.">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <FieldGroup label="Time zone">
                  <Select defaultValue="(UTC+05:00) Pakistan Standard Time">
                ...
                <FieldGroup label="Date format">
                  <Select defaultValue="DD/MM/YYYY"><option>DD/MM/YYYY</option><option>YYYY-MM-DD</option><option>MM/DD/YYYY</option></Select>
                </FieldGroup>
                <FieldGroup label="Currency">
                  <Select defaultValue="PKR — Pakistani Rupee"><option>PKR — Pakistani Rupee</option><option>AED — UAE Dirham</option><option>USD — US Dollar</option></Select>
                </FieldGroup>
              </div>
            </GlassCard>
          </section>
          ...
          <section id="notifications">
            <GlassCard title="Notifications" desc="Choose what you want to hear about.">
              <div className="space-y-3">
                <Toggle label="Email notifications"   desc="Order updates, stock alerts and customer inquiries." checked={notif.email} onChange={v => setNotif({ ...notif, email: v })} />
                <Toggle label="Push notifications"    desc="Real-time alerts on your devices." checked={notif.push} onChange={v => setNotif({ ...notif, push: v })} />
                <Toggle label="Marketing emails"      desc="Tips, wellness campaigns and performance." checked={notif.marketing} onChange={v => setNotif({ ...notif, marketing: v })} />
              </div>
            </GlassCard>
          </section>
            ...
            <GlassCard title="Security" desc="Keep your account safe.">
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  ...
                  <button type="button" className="h-9 px-3.5 rounded-xl bg-card border border-border hover:bg-secondary text-xs font-semibold text-foreground transition-colors shadow-sm">Update password</button>
                </div>
