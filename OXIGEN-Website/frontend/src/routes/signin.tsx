...
function SignInPage() {
  ...
  useEffect(() => {
    if (user) navigate({ to: user.user_type === "System User" ? "/admin" : "/dashboard" });
  }, [user, navigate]);

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    const loggedIn = await signIn(email, password);
    if (loggedIn) navigate({ to: loggedIn.user_type === "System User" ? "/admin" : "/dashboard" });
  };
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-1 top-1/2 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-lg text-muted-foreground hover:text-ink focus:outline-none"
