...
export const Route = createFileRoute("/offers/$bannerId")({
  head: ({ params }) => ({
    meta: [
      { title: `Featured Offers — OxiGen Pakistan` },
      {
        name: "description",
        content: `Explore featured products from this OxiGen banner offer. Premium quality supplements, delivered across Pakistan.`,
      },
    ],
  }),
  component: BannerOffersPage,
});
...
function BannerOffersPage() {
  ...
  const [bannerTitle, setBannerTitle] = useState("Featured Offers");

  useEffect(() => {
    setLoading(true);
    setError(false);

    // Step 1: Fetch banners to resolve the banner id + its linked product item_codes
    fetch(`${API_BASE}/banners`)
      ...
      .then((json) => {
        const banners: ApiBanner[] = json.data || [];
        const banner = banners.find((b) => b.id === bannerId);
        if (!banner) throw new Error("Banner not found");
        setBannerTitle(banner.title || "Featured Offers");
        const linkedCodes = new Set(
          (banner.products || []).map((p) => p.item_code).filter(Boolean)
        );

        // Step 2: Fetch full product details and filter to the banner's links
        return fetch(`${API_BASE}/items`).then((r) => {
          if (!r.ok) throw new Error();
          return r.json();
        }).then((itemsJson) => {
          const allItems: FullProduct[] = itemsJson.data || [];
          return allItems.filter((p) => linkedCodes.has(p.item_code));
        });
      })
      .then((matched) => {
        setProducts(matched);
  ...
  return (
    <SiteLayout>
      ...
      <PageHeader
        ...
        title={bannerTitle}
        sub={`Products featured in this OxiGen banner offer — quality ingredients, transparent formulations.`}
      />
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-7 md:grid-cols-3">
            {products.map((p, i) => {
              ...
              const img = getProductImage(p.image || p.website_image || "");
